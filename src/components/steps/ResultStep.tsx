...
import { ArrowLeft, Copy, Check, MapPin, Share2, Wallet, ArrowUpRight, ArrowDownLeft } from 'lucide-react';
...
export function ResultStep({ fullCalculation, participants, settlements, onBack, onReset }: ResultStepProps) {
  ...
  const handleCopy = async () => {
    const text = generateWhatsAppText(fullCalculation, settlements, language);
    try {
        ...
        {fullCalculation.outbound && fullCalculation.outbound.totalCost > 0 && (
          <div className="bg-primary/5 border border-primary/10 rounded-lg sm:rounded-xl p-3 sm:p-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-bold">{formatCurrency(fullCalculation.outbound.totalCost, language)}</span>
            </div>
            {outboundPayer && (
        ...
        {fullCalculation.return && fullCalculation.return.totalCost > 0 && (
          <div className="bg-accent/5 border border-accent/10 rounded-lg sm:rounded-xl p-3 sm:p-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-bold">{formatCurrency(fullCalculation.return.totalCost, language)}</span>
            </div>
            {returnPayer && (
              <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
                <Wallet className="w-3 h-3" />
      ...
      <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
        ...
        {sortedCosts.map((cost, index) => (
          <div
            ...
            className={cn(
              "bg-card rounded-lg sm:rounded-xl p-3 sm:p-4 card-shadow animate-slide-in",
            )}
          ...
          >
            <div className="flex items-center justify-between mb-2 sm:mb-3">
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                  <p className="text-sm sm:text-base font-semibold text-foreground">{cost.participantName}</p>
            ...
            <div className="space-y-1.5 sm:space-y-2 pt-2 sm:pt-3 border-t border-border">
              {cost.legDetails.map((leg, legIndex) => (
                <div
                  key={legIndex}
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    {leg.sharedWith > 1 && (
                      <span className="text-[10px] sm:text-xs bg-muted px-1.5 sm:px-2 py-0.5 rounded-full">
                        ÷{leg.sharedWith}
                      </span>
                    )}
